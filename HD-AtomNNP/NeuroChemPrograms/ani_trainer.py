    ...
    def train_network(self,LR,LA,CV,ST,PS=1):
        ...
        while LR > CV:
            ...
            while Toler is not 0:
                ...
                if Epoch%PS == 0:
                    self.tr_lc.append(hdn.hatokcal * np.sqrt(train_cost))
                    self.va_lc.append(hdn.hatokcal * np.sqrt(valid_cost))

                    print('Epoch(', str(Epoch).zfill(4), ') Errors:',
                          "{:7.3f}".format(hdn.hatokcal * np.sqrt(train_cost)), ':',
                          "{:7.3f}".format(hdn.hatokcal * np.sqrt(valid_cost)), ':',
...
# Network 1 Files
wkdir = '/home/jujuman/Research/SingleNetworkTest/train_05/'
cnstf = 'rHCNO-4.6A_16-3.1A_a4-8.params'
saenf = 'sae_6-31gd.dat'
nfdir = 'networks/'

# Data Dir
datadir = '/home/jujuman/Research/SingleNetworkTest/testcache/'
testdata = datadir + 'testset/testset.h5'
trainh5 = wkdir + 'ani_red_c02.h5'

# Test data
test_files = ["/home/jujuman/Research/ANI-DATASET/h5data/ani-gdb-c01.h5",
...
d = dict({'wkdir'         : wkdir,
          'sflparamsfile' : cnstf,
          'ntwkStoreDir'  : '/home/jujuman/Research/SingleNetworkTest/train_05/networks/',
...
while aani.get_Nbad() > 100:
    ...
    # Train network
    tr.train_network(LR, LA, CV, ST, 50)
    ...
    aani.add_bad_data(wkdir+cnstf, wkdir+saenf, wkdir+nfdir, GPU, True, P=0.1 + inc * 0.05, M=M)
...
# Train network
tr.train_network(LR, LA, CV, ST, 250)
