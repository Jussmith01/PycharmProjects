...
def printenergy(a=bz,b=mdcrd,d=dyn,t=temp,disCH=distCH,disCC=distCC):  # store a reference to atoms in the
    """Function to print the potential, kinetic and total energy."""
    epot = a.get_potential_energy() / len(a)
    ekin = a.get_kinetic_energy() / len(a)
    print('Step %i - Energy per atom: Epot = %.3feV  Ekin = %.3feV (T=%3.0fK)  '
          'Etot = %.3feV Time: %.3fs' % (d.get_number_of_steps(),epot, ekin, ekin / (1.5 * units.kB), epot + ekin, time.time()))
...
dfftCH = np.abs(np.fft.rfft(distCH))/float(len(distCC))
freqCH = np.fft.rfftfreq(distCH.shape[-1])

distCC = np.array(distCC)
distCC = distCC - distCC.mean()
dfftCC = np.abs(np.fft.rfft(distCC))/float(len(distCC))
...
plt.plot(freqCH, dfftCH, freqCC, dfftCC)
